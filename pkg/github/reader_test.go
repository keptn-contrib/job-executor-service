...
// For a POC is this ok ... kinda:
const trivyActionFile string = `name: 'Aqua Security Trivy'
...
func TestDownloadFileFromGithub(t *testing.T) {

	err, bytes := readActionYamlFromGithub("aquasecurity/trivy-action")
	assert.NilError(t, err)

	assert.Equal(t, string(bytes), trivyActionFile)
}

func TestUnmarshalActionYaml(t *testing.T) {

	err, action := GetActionYaml("aquasecurity/trivy-action")

	assert.NilError(t, err)
	assert.Check(t, action != nil)
	assert.Equal(t, action.Name, "Aqua Security Trivy")
}
